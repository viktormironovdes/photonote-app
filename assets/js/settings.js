...
async function shareAsZip(jsonString, filename, title) {
    try {
        ...
        // 3. Упаковываем в ZIP (если плагин доступен)
        if (isZipAvailable()) {
            const Zip = window.Capacitor.Plugins.Zip;
            const result = await Zip.zip({ files: files });
            zipBase64 = result.base64Data;
        } else {
            // Запасной вариант: используем JSZip на клиенте (если нужно)
            // Пока просто используем обычный JSON
            zipBase64 = jsonBase64;
        }
        
        // 4. Отправляем через FileSharer.share()
        if (isFileSharerAvailable()) {
            ...
            downloadJsonString(jsonString, filename.replace('.zip', '.json'));
        }
    } catch (error) {
        console.error('❌ Ошибка при создании ZIP:', error);
        // Если ZIP не сработал — пробуем обычную отправку
        fallbackShareJson(jsonString, filename.replace('.zip', '.json'), title);
    }
}

// ================================================================
// ЗАПАСНОЙ ВАРИАНТ (без ZIP)
// ================================================================

function fallbackShareJson(jsonString, filename, title) {
    if (isFileSharerAvailable()) {
        const FileSharer = window.Capacitor.Plugins.FileSharer;
...
async function importFromZip(event) {
    ...
    try {
        ...
        // 2. Проверяем, ZIP ли это (есть плагин)
        if (isZipAvailable() && file.name.endsWith('.zip')) {
            const Zip = window.Capacitor.Plugins.Zip;
            const result = await Zip.unzip({ base64Data: base64Data });
            if (result.files && result.files.length > 0) {
                // Берём первый файл из архива (предполагаем, что это data.json)
                const dataFile = result.files.find(f => f.filename === 'data.json' || f.filename.endsWith('.json'));
                if (dataFile) {
                    jsonString = atob(dataFile.base64Data);
                }
            }
        } else {
            // Если не ZIP — пробуем читать как JSON напрямую
...
function exportAllData() {
    ...
    const filename = `phonote_all_data_${getTodayStr()}.zip`;
...
// ================================================================
// ЭКСПОРТ ОТДЕЛЬНЫХ СУЩНОСТЕЙ (с ZIP)
// ================================================================
...
function exportReferences() {
    ...
    const jsonString = JSON.stringify(data, null, 2);
    const filename = `phonote_references_${getTodayStr()}.zip`;
...
function exportSchemes() {
    ...
    const jsonString = JSON.stringify(data, null, 2);
    const filename = `phonote_schemes_${getTodayStr()}.zip`;
...
function exportEquipment() {
    ...
    const jsonString = JSON.stringify(data, null, 2);
    const filename = `phonote_equipment_${getTodayStr()}.zip`;
...
function exportCheatsheets() {
    ...
    const jsonString = JSON.stringify(data, null, 2);
    const filename = `phonote_cheatsheets_${getTodayStr()}.zip`;
